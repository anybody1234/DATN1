...
Yêu cầu phát video bài giảng có theo dõi tiến độ và tự động chuyển sang quiz khi xem hết video (Chương \ref{chapter:Related_works}) đòi hỏi truy cập được các sự kiện phát video (bắt đầu, tạm dừng, kết thúc) và thời lượng video -- điều mà một thẻ \texttt{<iframe>} nhúng tĩnh không cung cấp. Em sử dụng YouTube IFrame Player API \cite{youtubeapi}, khởi tạo video qua \texttt{YT.Player} (vẫn giữ bộ điều khiển gốc của YouTube) và lắng nghe sự kiện \texttt{onStateChange} để phát hiện khi video kết thúc (\texttt{ENDED}), cũng như đọc thời lượng video (\texttt{getDuration()}) khi cần.
